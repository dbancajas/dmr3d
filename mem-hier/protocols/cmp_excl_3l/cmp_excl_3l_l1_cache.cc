...
// RCSID("$Id: cmp_excl_3l_l1_cache.cc,v 1.1.2.12 2006/03/15 19:38:31 pwells Exp $");
...
//
void
cmp_excl_3l_l1_line_t::init(cache_t *_cache, addr_t _idx, uint32 _lsize)
{
...
void
cmp_excl_3l_l1_line_t::wb_init(addr_t tag, addr_t idx, line_t *cache_line)
{
	FAIL;
}

void
cmp_excl_3l_l1_line_t::reinit(addr_t _tag)
{
	line_t::reinit(_tag);
	state = StateInvalid;
}	
...
bool
cmp_excl_3l_l1_line_t::is_free()
{
	return (state == StateInvalid);
}	
	
bool
cmp_excl_3l_l1_line_t::can_replace()
{
...
bool
cmp_excl_3l_l1_line_t::is_stable()
{
	// Same as can_replace
	return (can_replace());
}
...
void
cmp_excl_3l_l1_line_t::set_state(uint32 _state)
{
	state = _state;
}
	
uint32
cmp_excl_3l_l1_line_t::get_state()
{
	return state;
}

void
cmp_excl_3l_l1_line_t::to_file(FILE *file)
{
...
void
cmp_excl_3l_l1_line_t::from_file(FILE *file)
{
	fscanf(file, "%u ", &state);
...
cmp_excl_3l_l1_cache_t::cmp_excl_3l_l1_cache_t(string name, cache_config_t *conf,
	uint32 _id, bool _icache, conf_object_t *_cpu, uint64 prefetch_alg)
	: tcache_t<cmp_excl_3l_l1_protocol_t> (name, 2, conf, _id, prefetch_alg), 
	  cpu(_cpu), icache(_icache)
	// Two links for this protocol
{ }

uint32
cmp_excl_3l_l1_cache_t::get_action(message_t *msg)
{
...
void
cmp_excl_3l_l1_cache_t::trigger(tfn_info_t *t, tfn_ret_t *ret)
{
...
mem_trans_t *
cmp_excl_3l_l1_cache_t::get_enqueued_trans(tfn_info_t *t, tfn_ret_t *ret)
{
...
send_invalidate_up(tfn_info_t *t, tfn_ret_t *ret)
{
	if (!icache) {
		...
		stall_status_t retval = send_message(links[cmp_excl_3l_l1_protocol_t::proc_link], msg);
		ASSERT(retval == StallNone);
	}
}
...
void
cmp_excl_3l_l1_cache_t::enqueue_request_mshr(tfn_info_t *t, tfn_ret_t *ret)
{
...
void
cmp_excl_3l_l1_cache_t::perform_mshr_requests(tfn_info_t *t, tfn_ret_t *ret)
{
	addr_t addr = get_line_address(t->address);
	ASSERT(addr == t->line->get_address());
	ASSERT(addr == t->message->address);
...
void
cmp_excl_3l_l1_cache_t::prefetch_helper(addr_t message_addr, mem_trans_t *trans,
	bool read)
{
...
const string cmp_excl_3l_l1_protocol_t::action_names[][2] = {
	{ "Replace", "" }, 
	{ "ProcLD", "" }, 
	{ "ProcSt", "" }, 
	{ "DataShared", "" }, 
...
const string cmp_excl_3l_l1_protocol_t::state_names[][2] = {
	{ "Unknown", "Unknown" },
...
const string cmp_excl_3l_l1_protocol_t::prot_name[] = 
	{"cmp_excl_3l_l1", "Two-level Uniprocessor L2 Cache"};
