...
/* $Id: split_msi_bus.cc,v 1.1.1.1.10.1 2005/03/10 17:46:32 pwells Exp $
 *
...
split_msi_bus_t<prot_sm_t, msg_t>::split_msi_bus_t(string name, uint32 num_up, uint32 num_down)
...
template <class prot_sm_t, class msg_t>
stall_status_t
split_msi_bus_t<prot_sm_t, msg_t>::message_arrival(message_t *message)
{
	...
	VERBOSE_OUT(verb_t::bus, "%10s: %02u @ %12llu 0x%016llx: message arrival  %s last data 0x%016llx @ %12llu\n",
	            get_cname(), msg->sender_id, external::get_current_cycle(), 
				msg->address, msg_t::names[msg->type][0].c_str(), addr_on_data_bus, last_data_cycle);
	...
	switch(mclass) {
		case RequestPendResp:
		   ...
		   if (last_addr_cycle == curr_cycle + msg_lat || 
			   (curr_cycle <= last_data_cycle && msg->address == addr_on_data_bus) ||
		       (curr_cycle == last_addr_cycle && msg->address == addr_on_addr_bus)) 
			   ret=StallPoll;
		   else {
			   ...
			   if (msg->type == msg_t::WriteBack) {   
				   ...
				   if (last_addr_cycle == curr_cycle + msg_lat || 
				      ...
				      (curr_cycle == last_addr_cycle && msg->address == addr_on_addr_bus)) 
					   ...
					   // check if an outstanding request is pending
					   if (acycle_it != busy_addr_cycle.end()) {
						   // Pending outstanding transaction on the same line
						   VERBOSE_OUT(verb_t::bus, "%10s: %02u @ %12llu 0x%016llx: BUS Pending Message()\n",
						   get_cname(), msg->sender_id, external::get_current_cycle(), msg->address);
						   ret  = StallSetEvent;
...
template <class prot_sm_t, class msg_t>
void
split_msi_bus_t<prot_sm_t, msg_t>::event_handler(_event_t *e)
{
	msg_t *msg = e->get_data();
	split_msi_bus_t *bus = e->get_context();
	delete e;
	tick_t curr_cycle = external::get_current_cycle();
	...
	if (mclass == Response || mclass == RequestNoPend) {
		if (bus->last_data_cycle < curr_cycle)
			bus->last_data_cycle = curr_cycle;
		bus->pend_print = true;
	...
	// store the address on the data bus
	if (msg->type == msg_t::WriteBack || mclass == Response)
		bus->addr_on_data_bus = msg->address;
	else {
		// Message of request type
		bus->addr_on_addr_bus = msg->address;
	}
...
template <class prot_sm_t, class msg_t>
void
split_msi_bus_t<prot_sm_t, msg_t>::to_file(FILE *file)
{
	// Output class name
	fprintf(file, "%s\n", typeid(this).name());
}
...
template <class prot_sm_t, class msg_t>
void
split_msi_bus_t<prot_sm_t, msg_t>::from_file(FILE *file)
{
