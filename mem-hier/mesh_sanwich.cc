...
/* $Id: mesh_sanwich.cc,v 1.1.2.1 2006/12/12 18:38:09 pwells Exp $
 *
...
// RCSID("$Id: mesh_sanwich.cc,v 1.1.2.1 2006/12/12 18:38:09 pwells Exp $");
...
mesh_router_t::message_arrival(message_t *message)
{
    ...
    VERBOSE_OUT(verb_t::links, "Mesh router (%u, %u) Msg: src %u dest %u, link %u, bw %u cut? %s\n",
        x, y, inmsg->get_source(), inmsg->get_dest(), link, bw_delay, 
		cutthrough ? "yes" : "no");

	ASSERT(links[link]);
		
	return links[link]->send(message, g_conf_cmp_interconnect_latency,
