...
class bank_algorithm_t : public csp_algorithm_t {
    private:
        
       map<uint32, uint32> vcpu_2_pcpu;
       map<uint32, uint32> next_vcpu_2_pcpu;
       map<uint32, uint32> vcpu_2_currentbank;
	   
       void schedule(uint32 i );
       
	   uint32 get_nearest_pcpu(uint32 bank);

	   uint32 get_best_pcpu_unip(mai_t *thread);
	   void schedule_mp_vcpus();
	   
	   
	   uint32 mp_scheduled_this_quantum;
       
    public:
    
       bank_algorithm_t(string name, hw_context_t **hwc,
