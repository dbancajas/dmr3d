...
// RCSID("$Id: hybrid_alg.cc,v 1.1.2.2 2006/03/27 15:41:02 kchak Exp $");
...
hybrid_algorithm_t::hybrid_algorithm_t(string name, hw_context_t **hwc,
...
void hybrid_algorithm_t::schedule(uint32 i)
{
...
mai_t * hybrid_algorithm_t::find_thread_for_ctxt(hw_context_t *ctxt, 
...
mai_t *hybrid_algorithm_t::useful_local_thread(hw_context_t *ctxt)
{
    mai_t *ret = 0;
...
hw_context_t *hybrid_algorithm_t::find_ctxt_for_thread(mai_t *thread, 
...
bool hybrid_algorithm_t::thread_yield(sequencer_t *seq, uint32 ctxt, mai_t *mai, 
...
mai_t *hybrid_algorithm_t::find_remote_thread(hw_context_t *ctxt, 
...
void hybrid_algorithm_t::read_checkpoint(FILE *file)
{
    for (uint32 i = 0; i < num_ctxt; i++)
    {
        uint32 count;
...
void hybrid_algorithm_t::write_checkpoint(FILE *file)
{
    for (uint32 i = 0; i < num_ctxt; i++)
    {
        list<mai_t *>::iterator it = hw_context[i]->wait_list.begin();
...
void hybrid_algorithm_t::silent_switch(sequencer_t *seq, uint32 ctxt, mai_t *mai)
{

}    
    
sequencer_t *hybrid_algorithm_t::handle_interrupt_thread(uint32 thread_id)
{
...
ts_yield_reason_t hybrid_algorithm_t::prioritize_yield_reason(ts_yield_reason_t new_val,
...
void hybrid_algorithm_t::initialize_vanilla_logical()
{
...
void hybrid_algorithm_t::initialize_vanilla_gang()
{
    ASSERT(num_threads % num_ctxt == 0);
...
bool hybrid_algorithm_t::steal_previous_vcpu(hw_context_t *ctxt, mai_t *& remote_mai)
{
...
mai_t *hybrid_algorithm_t::force_steal_thread(hw_context_t *ctxt)
{
