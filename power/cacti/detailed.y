...
<table>
 ...
 <tr>
    <td></td>
	<td>Change Tag</td>
    <td colspan="2"><input name="changetag" type="radio" size="10" value="0" <%=get_checked(formValues.get("changetag",""),"0")%> />No	
	                <input name="changetag" type="radio" size="10" value="1" <%=get_checked(formValues.get("changetag",""),"1")%> />Yes
					 <span class="error"><%=formErrors.get("changetag","")%></span></td>
 </tr>
 <tr>
    <td></td>
	<td>Nr. of Bits per Tag</td>
    <td colspan="2"><input name="tagbits" type="text" size="10" value="<%=self.escape(formValues.get("tagbits",""))%>" /> <span class="error"><%=formErrors.get("tagbits","")%></span></td>
 </tr>
 <tr>
    <td></td>
	<td>Type of Cache</td>
    <td colspan="2"><input name="access_mode" type="radio" size="10" value="0" <%=get_checked(formValues.get("access_mode",""),"0")%> />Normal
	                <input name="access_mode" type="radio" size="10" value="1" <%=get_checked(formValues.get("access_mode",""),"1")%> />Serial
					<input name="access_mode" type="radio" size="10" value="2" <%=get_checked(formValues.get("access_mode",""),"2")%> />Fast
					 <span class="error"><%=formErrors.get("access_mode","")%></span></td>
 </tr>
...
<tr>
  ...
  <tr>
    ...
    <td><input type="hidden" name="action" value="submit" />
		...
		<input type="hidden" name="detailed" value="detailed_cache" />
...
<table>
	 ...
	 <td>
		 ...
		 Total Cache Size (bytes):<%=self.Request.cacti_output.params.cache_size*self.Request.cacti_output.result.subbanks%><br>
		 Size in bytes of bank:<%=self.Request.cacti_output.params.cache_size*self.Request.cacti_output.result.subbanks/self.Request.cacti_output.result.subbanks%><br>
		 Number of sets per bank:<%=self.Request.cacti_output.params.number_of_sets%><br> 
		 ...
		 Read/Write Ports per bank:<%=self.Request.cacti_output.params.num_readwrite_ports%><br> 
		 Read Ports per bank:<%=self.Request.cacti_output.params.num_read_ports%><br>
		 Write Ports per bank:<%=self.Request.cacti_output.params.num_write_ports%><br> 
