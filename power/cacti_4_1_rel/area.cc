...
static basic_circuit_t *bct = new basic_circuit_t();
...
area_type
area_t::inverter_area (double Widthp,double Widthn)
{
  double Width_n, Width_p;
  area_type invarea;
  int foldp = 0, foldn = 0;
  if (Widthp > 10.0 / circuit_param_t::cparam_FUDGEFACTOR)
    {
      Widthp = Widthp / 2, foldp = 1;
    }
  if (Widthn > 10.0 / circuit_param_t::cparam_FUDGEFACTOR)
    {
      Widthn = Widthn / 2, foldn = 1;
    }
  invarea.height = Widthp + Widthn + area_param_t::aparam_Widthptondiff + 2 * area_param_t::aparam_Widthtrack;
  Width_n =
    (foldn) ? (3 * area_param_t::aparam_Widthcontact +
	       2 * (area_param_t::aparam_Wpoly + 2 * area_param_t::aparam_ptocontact)) : (2 * area_param_t::aparam_Widthcontact + area_param_t::aparam_Wpoly +
						2 * area_param_t::aparam_ptocontact);
  Width_p =
    (foldp) ? (3 * area_param_t::aparam_Widthcontact +
	       2 * (area_param_t::aparam_Wpoly + 2 * area_param_t::aparam_ptocontact)) : (2 * area_param_t::aparam_Widthcontact + area_param_t::aparam_Wpoly +
						2 * area_param_t::aparam_ptocontact);
  invarea.width = CACTI_MAX (Width_n, Width_p);
  return (invarea);
}

area_type
area_t::subarraymem_area (int C,int B,int A,int Ndbl,int Ndwl,double Nspd,int RWP,int ERP,int EWP,int NSER,double techscaling_factor)	/* returns area of subarray */
{
  area_type memarea;
  ...
  memarea.height = ceil((double)(noof_rows)/16.0)*area_param_t::aparam_stitch_ramv+
  (area_param_t::aparam_BitHeight1x1+area_param_t::aparam_Widthtrack*2*(RWP+ERP+EWP-1))*noof_rows;
  memarea.width =
  noof_colns*(area_param_t::aparam_BitWidth1x1+
      (area_param_t::aparam_Widthtrack*2*(RWP+(ERP-NSER)+EWP-1)+
         area_param_t::aparam_Widthtrack*NSER));
  /* dt was : memarea.height =
  ceil((double)(noof_rows)/16.0)*area_param_t::aparam_stitch_ramv+(circuit_param_t::cparam_BitHeight16x2+2*area_param_t::aparam_Widthtrack*2*(RWP+ERP+EWP-1))*ceil((double)(noof_rows)/2.0);
  memarea.width =
  ceil((double)(noof_colns)/16.0)*(BitWidth16x2+16*(area_param_t::aparam_Widthtrack*2*(RWP+(ERP-NSER)+EWP-1)+Widthtrack*NSER));
  now : use single cell for width and height */
  
  //area_param_t::aparam_area_all_dataramcells = //Ndwl * Ndbl * calculate_area  (memarea, techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_param_t::aparam_area_all_dataramcells = Ndwl * Ndbl * memarea.height *
    memarea.width * CONVERT_TO_MMSQUARE; return (memarea);
}

area_type
area_t::decodemem_row (int C,int B,int A,int Ndbl,double Nspd,int Ndwl,int RWP,int ERP,int EWP)	/* returns area of post decode */
{
  ...
  */
  ...
  //noof_colns = 8 * B * A * Nspd / Ndwl;
  noof_colns = (int) (8 * B * A * Nspd / Ndwl + EPSILON);
  desiredrisetime = circuit_param_t::cparam_krise * log ((double) (noof_colns)) / 2.0;
  Cline = (2 * circuit_param_t::cparam_Wmemcella * circuit_param_t::cparam_Leff * circuit_param_t::cparam_Cgatepass + circuit_param_t::cparam_Cwordmetal) * noof_colns;
  Rpdrive = desiredrisetime / (Cline * log (VSINV) * -1.0);
  psize = circuit_param_t::cparam_Rpchannelon / Rpdrive;
  ...
  switch (numstack)
    {
    case 1:
      decodeNORwidth = area_param_t::aparam_WidthNOR1;
      break;
    case 2:
      decodeNORwidth = area_param_t::aparam_WidthNOR2;
      break;
    case 3:
      decodeNORwidth = area_param_t::aparam_WidthNOR3;
      break;
    case 4:
      decodeNORwidth = area_param_t::aparam_WidthNOR4;
      break;
    case 5:
      decodeNORwidth = area_param_t::aparam_WidthNOR4;
      break;
  ...
  decinv = inverter_area (circuit_param_t::cparam_Wdecinvp, circuit_param_t::cparam_Wdecinvn);
  worddriveinv = inverter_area (psize, nsize);
  /*
	was: postdecodearea.height = (circuit_param_t::cparam_BitHeight16x2+2*area_param_t::aparam_Widthtrack*2*(RWP+ERP+EWP-1));
  */
  postdecodearea.height = (2*area_param_t::aparam_BitHeight1x1+2*area_param_t::aparam_Widthtrack*2*(RWP+ERP+EWP-1));
...
area_type
area_t::predecode_area (double noof_rows,int RWP,int ERP,int EWP)
...

{
  ...
  switch (N3to8)
    {
    case 1:
      predecode_temp.height = area_param_t::aparam_Predec_height1;
      predecode_temp.width = area_param_t::aparam_Predec_width1;
      break;
    case 2:
      predecode_temp.height = area_param_t::aparam_Predec_height2;
      predecode_temp.width = area_param_t::aparam_Predec_width2;
      break;
    case 3:
      predecode_temp.height = area_param_t::aparam_Predec_height3;
      predecode_temp.width = area_param_t::aparam_Predec_width3;
      break;
    case 4:
      predecode_temp.height = area_param_t::aparam_Predec_height4;
      predecode_temp.width = area_param_t::aparam_Predec_width4;
      break;
    case 5:
      predecode_temp.height = area_param_t::aparam_Predec_height5;
      predecode_temp.width = area_param_t::aparam_Predec_width5;
      break;
    case 6:
      predecode_temp.height = area_param_t::aparam_Predec_height6;
      predecode_temp.width = area_param_t::aparam_Predec_width6;
      break;
    default:
...
//to postdecode_area
area_type
area_t::postdecode_area (int noof_rows,int RWP,int ERP,int EWP)
{
  ...
  decinverter = inverter_area (circuit_param_t::cparam_Wdecinvp, circuit_param_t::cparam_Wdecinvn);
  //v4.1: noof_rows can be less than 1 now since because Nspd can be a fraction. When
  //noof_rows is less than 1 making N3to8 1. This is not clean and needs to be fixed later. 
  if(noof_rows < 1) 
	numstack = 1;
  ...
  postdecode.height =
    (circuit_param_t::cparam_BitHeight + area_param_t::aparam_Widthtrack * 2 * (RWP + ERP + EWP - 1)) * noof_rows;
  postdecode.width =
...
area_type
area_t::colmux (int Ndbl,double Nspd,int RWP,int ERP,int EWP,int NSER)	/* gives the height of the colmux */
{
  area_type colmux_area;
  colmux_area.height =
  (2 * circuit_param_t::cparam_Wiso + 3 * (2 * area_param_t::aparam_Widthcontact + 1 / circuit_param_t::cparam_FUDGEFACTOR)) * (RWP + ERP + EWP);//Shyam: Need to understand what the +1 is for
  colmux_area.width =
  (circuit_param_t::cparam_BitWidth + area_param_t::aparam_Widthtrack * 2 * (RWP + (ERP - NSER) + EWP - 1) +
     area_param_t::aparam_Widthtrack * NSER);
  return (colmux_area);
}

area_type
area_t::precharge (int Ndbl,double Nspd,int RWP,int ERP,int EWP,int NSER)
{
  area_type precharge_area;
  if (Ndbl * Nspd > 1)
    {
      precharge_area.height =
	(circuit_param_t::cparam_Wbitpreequ + 2 * area_param_t::aparam_Wbitdropv + area_param_t::aparam_Wwrite + 2 * (2 * area_param_t::aparam_Widthcontact + 1 / circuit_param_t::cparam_FUDGEFACTOR) +
	 3 * area_param_t::aparam_Widthptondiff) * 0.5 * (RWP + EWP);
      precharge_area.width =
      2 * (circuit_param_t::cparam_BitWidth + area_param_t::aparam_Widthtrack * 2 * (RWP + (ERP - NSER) + EWP - 1) +
	     area_param_t::aparam_Widthtrack * NSER);
    }
  else
    {
      precharge_area.height =
	(circuit_param_t::cparam_Wbitpreequ + 2 * area_param_t::aparam_Wbitdropv + area_param_t::aparam_Wwrite + 2 * (2 * area_param_t::aparam_Widthcontact + 1 / circuit_param_t::cparam_FUDGEFACTOR) +
	 3 * area_param_t::aparam_Widthptondiff) * (RWP + EWP);
      precharge_area.width =
      circuit_param_t::cparam_BitWidth + area_param_t::aparam_Widthtrack * 2 * (RWP + (ERP - NSER) + EWP - 1) +
	area_param_t::aparam_Widthtrack * NSER;
    }
...
area_type
area_t::senseamp (int Ndbl,double Nspd,int RWP,int ERP,int EWP,int NSER)
{
  area_type senseamp_area;
  if (Ndbl * Nspd > 1)
    {
      senseamp_area.height = 0.5 * area_param_t::aparam_SenseampHeight * (RWP + ERP);
      senseamp_area.width =
      2 * (circuit_param_t::cparam_BitWidth + area_param_t::aparam_Widthtrack * 2 * (RWP + (ERP - NSER) + EWP - 1) +
	     area_param_t::aparam_Widthtrack * NSER);
    }
    ...
    {
      senseamp_area.height = area_param_t::aparam_SenseampHeight * (RWP + ERP);
      senseamp_area.width =
      circuit_param_t::cparam_BitWidth + area_param_t::aparam_Widthtrack * 2 * (RWP + (ERP - NSER) + EWP - 1) +
	area_param_t::aparam_Widthtrack * NSER;
    }
...
/* define area_param_t::aparam_OutdriveHeight OutdriveWidth area_param_t::aparam_DatainvHeight DatainvWidth */

area_type
area_t::subarraytag_area (int baddr,int C,int B,int A,int Ntdbl,int Ntdwl,int Ntspd,double NSubbanks,int RWP,int ERP,
		  int EWP,int NSER,double techscaling_factor)	/* returns area of subarray */
{
  ...
  if(!force_tag) {
	    //v4.1: Fixing double->int type conversion problems. EPSILON is added below to make sure
        //the final int value is the correct one 
		//Tagbits = ADDRESS_BITS + EXTRA_TAG_BITS-(int)bct->logtwo((double)C)+(int)bct->logtwo((double)A)-(int)(bct->logtwo(NSubbanks));
  ...
  tagarea.height = ceil((double)(noof_rows)/16.0)*area_param_t::aparam_stitch_ramv
    +(area_param_t::aparam_BitHeight1x1+area_param_t::aparam_Widthtrack*2*(RWP+ERP+EWP-1))*noof_rows;
  tagarea.width  = noof_colns*(area_param_t::aparam_BitWidth1x1 +
      (area_param_t::aparam_Widthtrack*2*(RWP+(ERP-conservative_NSER)+EWP-1)
      + area_param_t::aparam_Widthtrack*conservative_NSER));
  /*
  was:
  tagarea.height = ceil((double)(noof_rows)/16.0)*area_param_t::aparam_stitch_ramv+(circuit_param_t::cparam_BitHeight16x2+2*area_param_t::aparam_Widthtrack*2*(RWP+ERP+EWP-1))*ceil((double)(noof_rows)/2.0);
  tagarea.width = ceil((double)(noof_colns)/16.0)*(BitWidth16x2+16*(area_param_t::aparam_Widthtrack*2*(RWP+(ERP-conservative_NSER)+EWP-1)+Widthtrack*conservative_NSER));
  now:
  using single cell for width and height
  */

  //area_param_t::aparam_area_all_tagramcells =
    //Ntdwl * Ntdbl * calculate_area (tagarea,
				    //techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_param_t::aparam_area_all_tagramcells =  Ntdwl * Ntdbl * tagarea.height *tagarea.width * CONVERT_TO_MMSQUARE;
  return (tagarea);
}

area_type
area_t::decodetag_row (int baddr,int C,int B,int A,int Ntdbl,int Ntspd,int Ntdwl,double NSubbanks,int RWP,int ERP,int EWP)	/* returns area of post decode */
{
  ...
  decinv = inverter_area (circuit_param_t::cparam_Wdecinvp, circuit_param_t::cparam_Wdecinvn);
  worddriveinv = inverter_area (circuit_param_t::cparam_Wdecinvp, circuit_param_t::cparam_Wdecinvn);
  /*
  postdecodearea.height = (circuit_param_t::cparam_BitHeight16x2+2*area_param_t::aparam_Widthtrack*2*(RWP+ERP+EWP-1));
  */
  postdecodearea.height = (2*area_param_t::aparam_BitHeight1x1+2*area_param_t::aparam_Widthtrack*2*(RWP+ERP+EWP-1));
  postdecodearea.width =
    (decodeNORwidth + decinv.height + worddriveinv.height) * (RWP + ERP +
...
area_type
area_t::comparatorbit (int RWP,int ERP,int EWP)
{
  area_type compbit_area;
  compbit_area.width = 3 * area_param_t::aparam_Widthcontact + 2 * (3 * area_param_t::aparam_Wpoly + 2 * area_param_t::aparam_ptocontact);
  compbit_area.height = (circuit_param_t::cparam_Wcompn + 2 * (2 * area_param_t::aparam_Widthcontact + 1 / circuit_param_t::cparam_FUDGEFACTOR)) * (RWP + ERP);
  return (compbit_area);
}

area_type
area_t::muxdriverdecode (int B,int b0,int RWP,int ERP,int EWP)
{
  int noof_rows;
  ...
  muxdrvdecode_area.width =
    predecode.height + postdecode.width + noof_rows * area_param_t::aparam_Widthtrack * (RWP +
								    ERP +
								    EWP);
  muxdrvdecode_area.height = CACTI_MAX (predecode.width, postdecode.height);
  return (muxdrvdecode_area);
}

area_type
area_t::muxdrvsig (int A,int B,int b0)		/* generates the 8B/b0*A signals */
{
  int noof_rows;
  ...
  outdrvsig_area.height =
    0.5 * (circuit_param_t::cparam_WmuxdrvNORn + circuit_param_t::cparam_WmuxdrvNORp) + 9 * area_param_t::aparam_Widthcontact + 0.5 * (circuit_param_t::cparam_Wmuxdrv3n +
								  circuit_param_t::cparam_Wmuxdrv3p) +
    area_param_t::aparam_Widthptondiff + 3 * area_param_t::aparam_Widthcontact; 
  outdrvsig_area.width =
    (3 * area_param_t::aparam_Widthcontact + 2 * (3 * area_param_t::aparam_Wpoly + 2 * area_param_t::aparam_ptocontact)) * noof_rows;
  switch (A)
    {
    case 1:
      muxdrvsig_area.height =
	outdrvsig_area.height + noof_rows * area_param_t::aparam_Widthtrack * 2 + A *area_param_t::aparam_Widthtrack;
      muxdrvsig_area.width =
	outdrvsig_area.width + noof_rows * area_param_t::aparam_Widthtrack + A *area_param_t::aparam_Widthtrack;
      break;
    case 2:
      muxdrvsig_area.height =
	outdrvsig_area.height * 2 + noof_rows * area_param_t::aparam_Widthtrack * 3 +
	A * area_param_t::aparam_Widthtrack;
      muxdrvsig_area.width =
	outdrvsig_area.width + noof_rows * area_param_t::aparam_Widthtrack + A *area_param_t::aparam_Widthtrack;
      break;
    case 4:
      muxdrvsig_area.height =
	outdrvsig_area.height * 2 + noof_rows * area_param_t::aparam_Widthtrack * 5 +
	A * area_param_t::aparam_Widthtrack;
      muxdrvsig_area.width =
	outdrvsig_area.width * 2 + noof_rows * area_param_t::aparam_Widthtrack + A *area_param_t::aparam_Widthtrack;
      break;
    case 8:
      muxdrvsig_area.height =
	outdrvsig_area.height * 2 + noof_rows * area_param_t::aparam_Widthtrack * 9 +
	A * area_param_t::aparam_Widthtrack;
      muxdrvsig_area.width =
	outdrvsig_area.width * 4 + noof_rows * area_param_t::aparam_Widthtrack + A *area_param_t::aparam_Widthtrack;
      break;
    case 16:
      muxdrvsig_area.height =
	outdrvsig_area.height * 4 + noof_rows * area_param_t::aparam_Widthtrack * 18 +
	A * area_param_t::aparam_Widthtrack;
      muxdrvsig_area.width =
	outdrvsig_area.width * 4 + noof_rows * area_param_t::aparam_Widthtrack + A *area_param_t::aparam_Widthtrack;
      break;
    case 32:
      muxdrvsig_area.height =
	outdrvsig_area.height * 4 + noof_rows * area_param_t::aparam_Widthtrack * 35 +
	2 * A * area_param_t::aparam_Widthtrack;
      muxdrvsig_area.width =
	2 * (outdrvsig_area.width * 4 + noof_rows * area_param_t::aparam_Widthtrack +
	     A * area_param_t::aparam_Widthtrack);
      break;
    default:
...
area_type
area_t::datasubarray (int C,int B,int A,int Ndbl,int Ndwl,double Nspd,int RWP,int ERP,int EWP,int NSER,
	      double techscaling_factor)
{
  ...
  datasubarray_area.height =
    ...
    senseamp_area.height + area_param_t::aparam_DatainvHeight * (RWP + EWP) +
    area_param_t::aparam_OutdriveHeight * (RWP + ERP);
...
area_type
area_t::datasubblock (int C,int B,int A,int Ndbl,int Ndwl,double Nspd,int SB,
    int b0,int RWP,int ERP,int EWP,int NSER,double techscaling_factor)
{
  ...
  colmuxtracks_rem = (Ndbl * Nspd > tracks_precharge_p) ?
    (Ndbl * Nspd - tracks_precharge_p) : 0;
  outrdrvtracks_rem =
    ((2 * B * A) / (b0) > tracks_outdrvselinv_p) ? ((2 * B * A) / (b0) -
 ...
 N3to8 =
  ...
  tracks_h =
    area_param_t::aparam_Widthtrack * (N3to8 * 8 * (RWP + ERP + EWP) +
		  (RWP + EWP) * colmuxtracks_rem + Ndbl * Nspd * ERP +
		  4 * outrdrvtracks_rem * (RWP + ERP) +
		  4 * writeseltracks_rem * (RWP + EWP) + (RWP + ERP +
							  EWP) * b0 / SB_);
  tracks_w = area_param_t::aparam_Widthtrack * (N3to8 * 8) * (RWP + ERP + EWP);
  datasubarray_area =
...
area_type
area_t::dataarray (int C,int B,int A,int Ndbl,int Ndwl,double Nspd,int b0,int RWP,int ERP,int EWP,int NSER,
	   double techscaling_factor)
{
  int SB, N3to8;
  ...
  data = b0 * (RWP + ERP + EWP) * area_param_t::aparam_Widthtrack;
  driver_select = (2 * RWP + ERP + EWP) * 8 * B * A / b0 * area_param_t::aparam_Widthtrack;
  colmux = Ndbl * Nspd * (RWP + EWP + ERP) * area_param_t::aparam_Widthtrack;
  predecode = (RWP + ERP + EWP) * N3to8 * 8 * area_param_t::aparam_Widthtrack;
  addresslines = ADDRESS_BITS * (RWP + ERP + EWP) * area_param_t::aparam_Widthtrack;
  ...
  datasubblock_area =
    datasubblock (C, B, A, Ndbl, Ndwl, Nspd, SB, b0, RWP, ERP, EWP, NSER,
		  techscaling_factor);
  //area_param_t::aparam_area_all_datasubarrays =
    //Ndbl * Ndwl * calculate_area (datasubarray_area,
				  //techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_param_t::aparam_area_all_datasubarrays = Ndbl * Ndwl * datasubarray_area.height * datasubarray_area.width * CONVERT_TO_MMSQUARE;
...
area_type
area_t::tagsubarray (int baddr,int C,int B,int A,int Ndbl,int Ndwl,double Nspd,
    double NSubbanks,int RWP,int ERP,int EWP,int NSER, double techscaling_factor)
{
  ...
  conservative_NSER = 0;
  tag_area = subarraytag_area (baddr, C, B, A, Ndbl, Ndwl,  Nspd, NSubbanks, RWP, ERP,
		    EWP, conservative_NSER, techscaling_factor);
  postdecode_area = decodetag_row (baddr, C, B, A, Ndbl,Nspd, Ndwl, NSubbanks, RWP, ERP,
		   EWP);
...
area_type
area_t::tagsubblock (int baddr,int C,int B,int A,int Ndbl,int Ndwl,double Nspd,double NSubbanks,int SB,int RWP,int ERP,int EWP,
...
area_type
area_t::tagarray (int baddr,int C,int B,int A,int Ndbl,int Ndwl,double Nspd,double NSubbanks,int RWP,int ERP,int EWP,int NSER,
	  double techscaling_factor)
{
  //int SB, CSB, N3to8, T;
  int SB, N3to8, T;
  ...
  //the final int value is the correct one 
  if(!force_tag) {
		//T = ADDRESS_BITS + EXTRA_TAG_BITS-(int)bct->logtwo((double)C)+(int)bct->logtwo((double)A)-(int)(bct->logtwo(NSubbanks));
		T = (int) (ADDRESS_BITS + EXTRA_TAG_BITS-bct->logtwo((double)C)+
  ...
  tag = T * (RWP + ERP + EWP) * area_param_t::aparam_Widthtrack;
  assoc = (RWP + ERP) * A * area_param_t::aparam_Widthtrack;
  colmux = Ndbl * Nspd * (RWP + EWP + ERP) * area_param_t::aparam_Widthtrack;
  predecode = (RWP + ERP + EWP) * N3to8 * 8 * area_param_t::aparam_Widthtrack;
  addresslines = ADDRESS_BITS * (RWP + ERP + EWP) * area_param_t::aparam_Widthtrack;
  ...
  tagsubblock_area =
    tagsubblock (baddr, C, B, A, Ndbl, Ndwl, Nspd, NSubbanks, SB, RWP, ERP,
		 EWP, conservative_NSER, techscaling_factor);
  //area_param_t::aparam_area_all_tagsubarrays =
    //Ndbl * Ndwl * calculate_area (tagsubarray_area,
				  //techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_param_t::aparam_area_all_tagsubarrays = Ndbl * Ndwl * tagsubarray_area.height * tagsubarray_area.width * CONVERT_TO_MMSQUARE;
...
void
area_t::area (int baddr,int b0,int Ndbl,int Ndwl,double Nspd,int Ntbl,int Ntwl,int Ntspd,double NSubbanks,parameter_type *parameters,
      arearesult_type *result)
{
 ...
 //if-else on area_param_t::aparam_pure_sram_flag added by Shyam so that tag array area not calculated in
  //pure SRAM mode
...
area_type
area_t::fadecode_row (int C,int B,int Ndbl,int RWP,int ERP,int EWP)	/*returns area of post decode */
{
  int numstack;
  ...
  switch (numstack)
    ...
    case 4:
      ...
      break;
    case 5:
      decodeNORwidth = area_param_t::aparam_WidthNOR5;
      break;
    case 6:
      decodeNORwidth = area_param_t::aparam_WidthNOR6;
      break;
    default:
      printf ("error:numstack=%d\n", numstack);
      printf ("Cacti does not support a series stack of %d transistors !\n",
  ...
  switch (numstack)
    ...
    case 3:
      ...
      break;
    case 4:
      firstinv = area_param_t::aparam_decNandWidth;
      break;
    case 5:
      firstinv = area_param_t::aparam_decNandWidth;
      break;
  ...
  /*
  was : postdecodearea.height = circuit_param_t::cparam_BitHeight16x2;
  */
  postdecodearea.height = 2*area_param_t::aparam_BitHeight1x1;
  postdecodearea.width =
...
area_type
area_t::fasubarray (int baddr,int C,int B,int Ndbl,int RWP,int ERP,int EWP,int NSER,double techscaling_factor)	/* returns area of subarray */
{
  ...
  precharge =
    circuit_param_t::cparam_Wbitpreequ + 2 * area_param_t::aparam_Wbitdropv + area_param_t::aparam_Wwrite + 2 * (2 * area_param_t::aparam_Widthcontact + 1 / circuit_param_t::cparam_FUDGEFACTOR) +
    3 * area_param_t::aparam_Widthptondiff;
  ...
  if ((RWP == 1) && (ERP == 0) && (EWP == 0))
	...
	{
	  widthoverhead = area_param_t::aparam_FAWidthIncrPer_first_r_port;
	  heightoverhead = area_param_t::aparam_FAHeightIncrPer_first_r_port;
	}
      else
	{
	  if ((RWP == 1) && (ERP == 0) && (EWP == 1))
	    {
	      widthoverhead = area_param_t::aparam_FAWidthIncrPer_first_rw_or_w_port;
	      heightoverhead = area_param_t::aparam_FAHeightIncrPer_first_rw_or_w_port;
	    }
	  else
	    {
	      if (RWP + EWP >= 2)
		{
		  widthoverhead =
		    area_param_t::aparam_FAWidthIncrPer_first_rw_or_w_port + (RWP + EWP -
							 2) *
		    area_param_t::aparam_FAWidthIncrPer_later_rw_or_w_port +
		    ERP * area_param_t::aparam_FAWidthIncrPer_later_r_port;
		  heightoverhead =
		    area_param_t::aparam_FAHeightIncrPer_first_rw_or_w_port + (RWP + EWP -
							  2) *
		    area_param_t::aparam_FAHeightIncrPer_later_rw_or_w_port +
		    ERP * area_param_t::aparam_FAHeightIncrPer_later_r_port;}
	      else
		{
		  if ((RWP == 0) && (EWP == 0))
		    {
		      widthoverhead =
			area_param_t::aparam_FAWidthIncrPer_first_r_port + (ERP -
						       1) *
			area_param_t::aparam_FAWidthIncrPer_later_r_port;
		      heightoverhead =
			area_param_t::aparam_FAHeightIncrPer_first_r_port + (ERP -
							1) *
			area_param_t::aparam_FAHeightIncrPer_later_r_port;;}
		  else
		    {
		      if ((RWP == 0) && (EWP == 1))
			{
			  widthoverhead = ERP * area_param_t::aparam_FAWidthIncrPer_later_r_port;
			  heightoverhead = ERP * area_param_t::aparam_FAHeightIncrPer_later_r_port;
			}
		      else
			{
			  if ((RWP == 1) && (EWP == 0))
  ...
  faramcell.height =
    ceil ((double) (noof_rowsdata) / 16.0) * area_param_t::aparam_stitch_ramv + (area_param_t::aparam_CAM2x2Height_1p +
							    2 *
							    heightoverhead) *
    ceil ((double) (noof_rowsdata) / 2.0);

  /*
  was: faramcell.width=(ceil((double)(noof_colnsdata)/16.0))*(BitWidth16x2+16*(area_param_t::aparam_Widthtrack*2*(RWP+(ERP-NSER)+EWP-1)+Widthtrack*NSER))+2*(HTagbits*((area_param_t::aparam_CAM2x2Width_1p+2*widthoverhead)-area_param_t::aparam_Widthcontact))+(BitWidth+Widthtrack*2*(RWP+ERP+EWP-1))+(area_param_t::aparam_FArowNANDWidth+area_param_t::aparam_FArowNOR_INVWidth)*(RWP+ERP+EWP);
  */
  faramcell.width=noof_colnsdata*(area_param_t::aparam_BitWidth1x1+
      (area_param_t::aparam_Widthtrack*2*(RWP+(ERP-NSER)+EWP-1)+
          area_param_t::aparam_Widthtrack*NSER))+2*(HTagbits*((area_param_t::aparam_CAM2x2Width_1p+2*widthoverhead)
          -area_param_t::aparam_Widthcontact))+
          (circuit_param_t::cparam_BitWidth+area_param_t::aparam_Widthtrack*2*(RWP+ERP+EWP-1))+
          (area_param_t::aparam_FArowNANDWidth+area_param_t::aparam_FArowNOR_INVWidth)*(RWP+ERP+EWP);

  FAarea.height =
    faramcell.height + precharge * (RWP + EWP) + area_param_t::aparam_SenseampHeight * (RWP + ERP) +
    area_param_t::aparam_DatainvHeight * (RWP + EWP) + area_param_t::aparam_OutdriveHeight * (RWP + ERP);
  ...
  //area_param_t::aparam_faarea_all_subarrays =
    //Ndbl * calculate_area (FAarea, techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_param_t::aparam_area_all_dataramcells = Ndbl * faramcell.height * faramcell.width * CONVERT_TO_MMSQUARE;
  area_param_t::aparam_faarea_all_subarrays = Ndbl * FAarea.height * FAarea.width * CONVERT_TO_MMSQUARE;
  return (FAarea);
}

area_type
area_t::faarea (int baddr,int b0,int C,int B,int Ndbl,int RWP,int ERP,int EWP,int NSER,double techscaling_factor)
{
 ...
 N3to8 =
  ...
  //v4.1: Fixing double->int type conversion problems. EPSILON is added below to make sure
  //the final int value is the correct one 
  //blocksel = CACTI_MAX (logtwo_area ((double) (B)), (8 * B) / b0);
  blocksel = CACTI_MAX ((logtwo_area ((double) (B)) + EPSILON), (8 * B) / b0);
  blocksel =
    (blocksel >
     tracks_outdrvfanand_p) ? (blocksel - tracks_outdrvfanand_p) : 0;

  fixed_tracks =
    area_param_t::aparam_Widthtrack * (1 * (RWP + EWP) + b0 * (RWP + ERP + EWP) +
		  Tagbits * (RWP + ERP + EWP) + blocksel * (RWP + ERP + EWP));
  predecode = area_param_t::aparam_Widthtrack * (N3to8 * 8) * (RWP + EWP);
...
void
area_t::fatotalarea (int baddr,int b0,int Ndbl,parameter_type *parameters,arearesult_type *faresult)
{
  area_type null_area;
...
void
area_t::area_subbanked (int baddr,int b0,int RWP,int ERP,int EWP,int Ndbl,int Ndwl,double Nspd,int Ntbl,int Ntwl,int Ntspd,
		double NSubbanks,parameter_type *parameters,area_type *result_subbanked,arearesult_type *result)
{
  ...
  area_param_t::aparam_area_all_dataramcells = 0.0;
  area_param_t::aparam_area_all_tagramcells = 0.0;
  area_param_t::aparam_aspect_ratio_data = 1.0;
  area_param_t::aparam_aspect_ratio_tag = 1.0;
  area_param_t::aparam_aspect_ratio_subbank = 1.0;
  area_param_t::aparam_aspect_ratio_total = 1.0;
  ...
  if (NSubbanks == 2)
	...
	(result_area.dataarray_area.width +
	 result_area.tagarray_area.width) * 2 + (ADDRESS_BITS +
						 circuit_param_t::cparam_BITOUT) * NSubbanks * (RWP +
									ERP +
									EWP);
    }
  if (NSubbanks == 4)
  ...
  if (NSubbanks == 8)
	...
	2 * (2 * result_area.dataarray_area.height +
	     2 * (RWP + ERP + EWP) * ADDRESS_BITS) + (ADDRESS_BITS +
						      circuit_param_t::cparam_BITOUT) * NSubbanks *
	(RWP + ERP + EWP);
    }
  ...
  if (NSubbanks > 8)
	...
	((result_area.
	 ...
	 (ADDRESS_BITS + circuit_param_t::cparam_BITOUT) * 16 * (RWP + ERP + EWP) * 0.25) +
	(ADDRESS_BITS + circuit_param_t::cparam_BITOUT) * 16 * (RWP + ERP + EWP) * 0.5;
      base_width =
	2 * (2 * result_area.dataarray_area.height +
	     2 * (RWP + ERP + EWP) * ADDRESS_BITS) + (ADDRESS_BITS +
						      circuit_param_t::cparam_BITOUT) * 16 * (RWP +
								      ERP +
								      EWP) *
	0.25;
      base_subbanks = 16;
      ...
      else
	{
	  for (i = 1; i <= iter_height; i++)
		...
		2 * (inter_height) + (ADDRESS_BITS +
				      circuit_param_t::cparam_BITOUT) * 4 * inter_subbanks * (RWP +
								      ERP +
								      EWP) *
		0.5;
	      inter_height = total_height;
      ...
      else
	{
	  if (htree % 2 == 0)
	      ...
	      for (i = 1; i <= iter_width; i++)
		{
		  total_width =
		    2 * (inter_width) + (ADDRESS_BITS +
					 circuit_param_t::cparam_BITOUT) * inter_subbanks * (RWP +
								     ERP +
								     EWP);
	      ...
	      for (i = 1; i <= iter_width_double; i++)
		{
	      ...
	      total_width +=
		(ADDRESS_BITS + circuit_param_t::cparam_BITOUT) * (RWP + ERP + EWP) * NSubbanks / 2;
	    }
	}
    }
  ...
  area_param_t::aparam_aspect_ratio_total = (result_subbanked->height / result_subbanked->width);

  area_param_t::aparam_aspect_ratio_total =
    (area_param_t::aparam_aspect_ratio_total >
     1.0) ? (area_param_t::aparam_aspect_ratio_total) : 1.0 / (area_param_t::aparam_aspect_ratio_total);


  if (area_param_t::aparam_aspect_ratio_total > temp_aspect)
    {
      result_subbanked->height = temp.height;
      result_subbanked->width = temp.width;
    }

  area_param_t::aparam_aspect_ratio_subbank =
    (result_area.dataarray_area.height /
     (result_area.dataarray_area.width + result_area.tagarray_area.width));
  area_param_t::aparam_aspect_ratio_subbank =
    (area_param_t::aparam_aspect_ratio_subbank >
     1.0) ? (area_param_t::aparam_aspect_ratio_subbank) : 1.0 / (area_param_t::aparam_aspect_ratio_subbank);
  area_param_t::aparam_aspect_ratio_total = (result_subbanked->height / result_subbanked->width);
  area_param_t::aparam_aspect_ratio_total =
    (area_param_t::aparam_aspect_ratio_total >
     1.0) ? (area_param_t::aparam_aspect_ratio_total) : 1.0 / (area_param_t::aparam_aspect_ratio_total);
...
int
area_t::data_organizational_parameters_valid (int B,int A,int C,int Ndwl,
    int Ndbl,double Nspd,char assoc,double NSubbanks)
{
	int data_rows = 0, data_columns = 0;
  /* don't want more than 8 subarrays for each of data/tag */

  if (assoc == 0) {
		if (Ndwl * Ndbl > MAXSUBARRAYS) {return (CACTI_FALSE);}
		/* add more constraints here as necessary */
		//v4.1: Number of rows per subarray is (C / (B * A * Ndbl * Nspd),
		//not (C / (8 * B * A * Ndbl * Nspd)
		//if (C / (8 * B * A * Ndbl * Nspd) <= 0) {return (CACTI_FALSE);}
		if (C / (B * A * Ndbl * Nspd) <= 0) {return (CACTI_FALSE);}
		if ((8 * B * A * Nspd / Ndwl) <= 0) {return (CACTI_FALSE);}
		...
		if(8 > data_rows) {return (CACTI_FALSE);}
		//dt: data side: number of rows should be less than 4k 
		if(2048 < data_rows) {return (CACTI_FALSE);}
		...
		if(16 > data_columns) {return (CACTI_FALSE);}
		//dt: data side: number of columns should be less than 1k 
		if(512 < data_columns) {return (CACTI_FALSE);}

		//Added by Shyam to specify a minimum 8KB mat (subarray) size in the data array
		//if((C <= 8192)&&((Ndwl > 1)||(Ndbl > 1))) return (CACTI_FALSE);
		//End of Shyam's minimum mat size change
		
  }
  else {
		if (C / (2 * B * Ndbl) <= 0) {return (CACTI_FALSE);}
		if (Ndbl > MAXDATAN) {return (CACTI_FALSE);}	
  }
...
int
area_t::tag_organizational_parameters_valid (int B,int A,int C,int Ntwl,int Ntbl,int Ntspd,char assoc,double NSubbanks)
{
	int temp = 0;
	double before = 0.0;
	int tagbits = 0;
	int tag_rows = 0, tag_columns = 0;
  /* don't want more than 8 subarrays for each of data/tag */

  if (assoc == 0) {
		if (Ntwl * Ntbl > MAXSUBARRAYS) {return (CACTI_FALSE);}
		/* add more constraints here as necessary */
		//v4.1: Number of rows per subarray is (C / (B * A * Ntbl * Ntspd)
		//not (C / (8 * B * A * Ntbl * Ntspd)
		//if (C / (8 * B * A * Ntbl * Ntspd) <= 0) {return (CACTI_FALSE);}
		if (C / (B * A * Ntbl * Ntspd) <= 0) {return (CACTI_FALSE);}
		if ((8 * B * A * Ntspd / Ntwl) <= 0) {return (CACTI_FALSE);}
		/*dt: Don't want ridicolously small arrays*/
		...
		if(8 > tag_rows) {return (CACTI_FALSE);}
		//dt: tag side: number of rows should be less than 1k 
		if(512 < tag_rows) {return (CACTI_FALSE);}
		// dt: tag side: number of column should be greater than 8 
		tagbits = ADDRESS_BITS+EXTRA_TAG_BITS;
		 //v4.1: Fixing double->int type conversion problems. EPSILON is added below to make sure
        //the final int value is the correct one 
		//before = bct->logtwo((double)C);
		before = (int) (bct->logtwo((double)C) + EPSILON);
		temp = (int)(before);
		tagbits -= temp;
		//tagbits += (int)bct->logtwo((double)A);
		//tagbits -= (int)(bct->logtwo(NSubbanks));
		tagbits += (int) (bct->logtwo((double)A) + EPSILON);
		tagbits -= (int)((bct->logtwo(NSubbanks)) + EPSILON);
		tag_columns = tagbits * A * Ntspd/Ntwl;
		if(8 > tag_columns) {return (CACTI_FALSE);}
		// dt: tag side: number of column should be less than 4k 
		if(512 < tag_columns) {return (CACTI_FALSE);}
		
		
  }
  else {
		if (Ntbl > MAXTAGN) {return (CACTI_FALSE);}
