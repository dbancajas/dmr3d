...
#
#Use IE or SVG Viewer to open "thermal_map.svg". May need to enable XML
#in your IE. Also, in Linux, ImageMagick 'convert' could be used to convert
#it to other file formats (eg `convert -font Helvetica file.svg file.pdf`).
#
#Inputs: 
#	(1) A floorplan file with the same format as the other HotSpot .flp files.
#	(2) A list of grid temperatures, with the format of each line: grid_number	grid_temperature
#	Example floorplan file (sample_map_flp) and the corresponding 40x40 grid 
#	temperature file (sample_grid_temp) are inlcuded in this release. The resulting
#	SVG figure (thermal_map.svg) is also included. 
#
#Note: grid temperatures can be obtained by dumping the grid indice and steady_g_temp[]
# (or g_temp[] for transient temperatures) to file "sample_grid_temp".
#
#IMPORTANT: modifications to this file when generating your own SVG thermal map:
# (1) Change $row and $col to the desired values, be consistent with the file "sample_grid_temp"
# (2) Instead of extracting max T and min T from the grid temperature file, you may want to manually 
#			specify the max T and min T to fixed values by setting $min_t and $max_t.
#
#Acknowledgement: The HotSpot developers would like to thank Joshua Rosenbluh at Grinnell 
#									College, Iowa, for his information and help on the SVG figures.
use strict;
...
sub usage () {
    ...
    print("<rows>           -- no. of rows in the grid (default 50)\n");
    print("<cols>           -- no. of columns in the grid (default 50)\n");
