...
void populate_R_model_block(block_model_t *model, flp_t *flp)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (model->config.block_omit_lateral) {
			...
			gx[i] = 1.0/getr(K_SI, flp->units[i].width / 2.0, flp->units[i].height * t_chip);
			gy[i] = 1.0/getr(K_SI, flp->units[i].height / 2.0, flp->units[i].width * t_chip);
		}

		/* at the interface layer	*/
		gx_int[i] = 1.0/getr(K_INT, flp->units[i].width / 2.0, flp->units[i].height * t_interface);
		gy_int[i] = 1.0/getr(K_INT, flp->units[i].height / 2.0, flp->units[i].width * t_interface);

		/* at the spreader layer	*/
		gx_sp[i] = 1.0/getr(K_CU, flp->units[i].width / 2.0, flp->units[i].height * t_spreader);
		gy_sp[i] = 1.0/getr(K_CU, flp->units[i].height / 2.0, flp->units[i].width * t_spreader);

		/* at the heatsink layer	*/
		gx_hs[i] = 1.0/getr(K_CU, flp->units[i].width / 2.0, flp->units[i].height * t_sink);
		gy_hs[i] = 1.0/getr(K_CU, flp->units[i].height / 2.0, flp->units[i].width * t_sink);
	}
	...
	for (i = 0; i < n; i++) {
		...
		g[i][IFACE*n+i]=g[IFACE*n+i][i]=2.0/getr(K_SI, t_chip, area);
 		/* vertical g's in the interface layer	*/
		g[IFACE*n+i][HSP*n+i]=g[HSP*n+i][IFACE*n+i]=2.0/getr(K_INT, t_interface, area);
		/* vertical g's in the spreader layer	*/
		g[HSP*n+i][HSINK*n+i]=g[HSINK*n+i][HSP*n+i]=2.0/getr(K_CU, t_spreader, area);
		...
		g_amb[i] = 1.0 / (getr(K_CU, t_sink, area) + r_amb);
...
void populate_C_model_block(block_model_t *model, flp_t *flp)
{
	...
	for (i = 0; i < n; i++) {
		...
		a[i] = getcap(SPEC_HEAT_SI, t_chip, area);
		/* C's from interface portion of the functional units to ground	*/
		a[IFACE*n+i] = getcap(SPEC_HEAT_INT, t_interface, area);
		/* C's from spreader portion of the functional units to ground	*/
		a[HSP*n+i] = getcap(SPEC_HEAT_CU, t_spreader, area);
		...
		a[HSINK*n+i] = getcap(SPEC_HEAT_CU, t_sink, area) + c_amb;
	}
